  ...
  <script language="javascript" type="text/javascript">
    $(function() {
      ...
      function Inserisci(costrutto, gruppo) {
        ...
        searchXML(lista_canali, '', '0');
        vision = '0';
        //console.log(lista_url,lista_grp,lista_canali);
        for (z = 0; z <= lista_canali.length - 1; z++) {
          var dt_tempo = new Date();
          var dt_now = dt_tempo.getTime();
          trovato = false
          in_onda = 0;
          if (canale !== null && typeof canale[lista_canali[z]] !== "undefined") {
            for (i = 0; i < canale[lista_canali[z]].length; i++) {
              if (canale[lista_canali[z]][i]['status'] === '1') {
                trovato = true;
                in_onda = i;
                i = canale[lista_canali[z]].length;
              }
            }
            if (trovato) {
              var tzstart = moment(canale[lista_canali[z]][in_onda]['start'], 'YYYYMMDDHHmmSS').subtract(0, 'minutes');
              start = new Date(tzstart);
              dt_start = start.getTime();
              var tzstop = moment(canale[lista_canali[z]][in_onda]['stop'], 'YYYYMMDDHHmmSS').subtract(0, 'minutes');
            ...
            if (vision === '1') {
              if (canale[lista_canali[z]][in_onda]["icon"] !== null) {
                //if (  canale[in_onda]["icon"].substr(0,4) === 'http') { path ="";} else {path="./image/picon_name/"; path="./image/picon_id/"}             
                $(".brand-logo").html("<div style='float:right;margin:5px;'><img width='100px'; style='margin-right:50px' src='" + canale[lista_canali[z]][in_onda]["icon"] + "'  onerror='this.onerror=null;this.src='" + canale[lista_canali[z]][in_onda]["icon"] + "'></div> " + canale[lista_canali[z]][in_onda]['channel']);
              }
              if (trovato) {
                $(".linea_principale").css({
                  "width": perc_visto + "%"
                });
                $("#info").text(escapeHtml(canale[lista_canali[z]][in_onda]['title']));
                $("#start_info").text(dateformat(start, "1"));
                $("#stop_info").text(dateformat(stop, "1"));
                trama = canale[lista_canali[z]][in_onda]['desc'];
                ...
                try {
                  $("#next_info").text(canale[lista_canali[z]][in_onda + 1]['title']);
                } catch (e) {}
              }
            }

            if (vision === '0') {
              // return canale[lista_canali[z]][in_onda];
            }
          } else if (vision === '1') {
            $(".brand-logo").text(canale[lista_canali[z]][in_onda]["channel"]);
          }
          ...
          p_header = "";
          if (z === 0) {
            p_header = "<i style='position:absolute;left:10px;top:12px;' class='material-icons prefix'>search</i><input type='search' class='white-text' style='margin-bottom:30px' placeholder='Search' id='s_canale'>"
          }
          aj_logo = "  ";
          p_canale = "<div style='margin-top:-10px' class='scelto'>" + lista_canali[z].toUpperCase() + "</div>";
          ...
          if (canale !== null && typeof canale[lista_canali[z]] !== "undefined") {
            p_logo = "<div style='position:absolute;left:2px'><img style='width:35px;margin-top:-10px' src='" + canale[lista_canali[z]][in_onda]['icon'] + "'  onerror=\"this.src='image/tv-orange.png'\"></div>";
          }
          ...
          if (trovato) {
            ...
            p_title = "<div class='orange-text' style='margin-bottom:-10px;'>" + canale[lista_canali[z]][in_onda]['title'] + "</div>";
          }

          $(".collection").append("<a href='#!' class='collection-item grey darken-4 white-text' cur_group= '" + lista_url[z] + "' cur_nome='" + lista_grp[z] + "' style='padding-left:50px;font-size:12px;' id='" + segment[3] + "'>" +
            p_header +
            p_logo +
            p_canale +
            p_orario +
            "<div class='progress  grey darken-3' style='width: 300px;margin:1px;'>" +
            p_progress +
            p_title +
            "</a>");
  ...
  <div id="main" style="padding:0;">
    ...
    <div class="blue-grey" style="height:130px;" id="container">
      ...
      <div id="tab_video" class="col s12">
        <div id="content" style="position:relative;top:75px" class="card blue-grey lighten-3">
